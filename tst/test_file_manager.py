...
from src.file_manager import load_users, save_user, get_user, \
                            delete_user, update_user, update_user_simple
...
class TestFileManager(unittest.TestCase):
    ...
    def test_load_users(self, mock_user, mock_json_load, mock_open, mock_listdir) -> None:
        """Test case for Load Users"""
        mock_listdir.return_value = ['user1.json']
        mock_json_load.return_value = {'username': 'user1',
                                       'password': 'pass1',
                                       'library': {'books': []}}
        result: list[User] = load_users()
    ...
    def test_save_user(self, mock_json_dumps, mock_encrypt, mock_open) -> None:
        ...
        save_user(mock_user)
        mock_open.assert_called_once_with(os.path.join('users',
                                        'encrypted_user.json'), 'x', encoding='utf-8')
    ...
    @patch('src.file_manager.json.load')
    @patch('src.file_manager.User', side_effect=lambda x: User(x))
    def test_get_user(self, mock_user, mock_json_load, mock_encrypt, mock_open, mock_listdir) -> None:
    ...
    @patch('src.file_manager.encrypt')
    def test_delete_user(self, mock_encrypt, mock_path_exists, mock_remove) -> None:
    ...
    def test_update_user(self, mock_save_user, mock_delete_user, mock_get_user) -> None:
    ...
    @patch('src.file_manager.delete_user')
    @patch('src.file_manager.save_user')
    def test_update_user_simple(self, mock_save_user, mock_delete_user) -> None:
